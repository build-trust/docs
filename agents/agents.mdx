...
Now let's customize the agent. In the code below we change the prompt so that the
agent is now an expert legal assistant instead of a pirate:
...
You'll again be connected to your agent that is running in the cloud on the 
Ockam serverless runtime. Interact with it and you'll see it now responds with
legal insight and not pirate talk.
