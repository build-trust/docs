...
1. It builds the code in `images/main` into a container image and pushes it to a container registry that is available to your Zone. It then deploys the Zone into your Cluster, in Ockam AI, based on the configuration that is specified above in `ockam.yaml`.
...
3. Finally it connects a REPL client to the REPL server over this private and secure portal.